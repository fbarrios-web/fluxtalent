...
function ResetPasswordPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    if (password !== confirm) { toast.error(t("Las contraseñas no coinciden")); return; }
    if (password.length < 8) { toast.error(t("Mínimo 8 caracteres")); return; }
    setLoading(true);
    try {
      ...
      toast.error(err.message ?? t("No se pudo actualizar la contraseña"));
    } finally {
      setLoading(false);
    }
  }
  ...
  return (
    <div className="grid min-h-screen place-items-center p-6">
      <div className="w-full max-w-sm">
        ...
        <h1 className="font-display text-3xl">{t("Restablecer contraseña")}</h1>
        <p className="mt-1 text-sm text-muted-foreground">
          {ready ? t("Ingresá tu nueva contraseña.") : t("Validando enlace...")}
        </p>
        ...
        <p className="mt-6 text-center text-sm text-muted-foreground">
          <Link to="/auth" className="font-medium text-primary hover:underline">{t("Volver al login")}</Link>
        </p>
      </div>
    </div>
  );
