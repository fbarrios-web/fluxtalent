    ...
    <div className="grid min-h-screen md:grid-cols-2">
      ...
      <div className="flex items-center justify-center p-6">
        <div className="w-full max-w-sm">
          ...
          {mode === "signup" && (
            <div className="mt-4 rounded-xl border border-primary/30 bg-primary/10 px-3 py-2 text-xs text-foreground">
              🎁 <b>Este mes:</b> 1 mes del plan Starter gratis. Sin tarjeta.
            </div>
          )}

          {inApp && (
            <p className="mt-4 rounded-lg border border-border bg-muted/50 px-3 py-2 text-xs text-muted-foreground">
              Si Google no te deja entrar desde Instagram, usá tu email abajo o abrí esta página en tu navegador (menú ⋯ → "Abrir en el navegador").
            </p>
          )}

          <Button onClick={handleGoogle} size="lg" className="mt-5 h-12 w-full text-base" disabled={loading}>
            <span className="mr-2 grid h-6 w-6 place-items-center rounded-full bg-background">
              <svg className="h-4 w-4" viewBox="0 0 24 24"><path fill="#4285F4" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.530-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"/><path fill="#34A853" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.930-6.160-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"/><path fill="#FBBC05" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"/><path fill="#EA4335" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"/></svg>
            </span>
          ...
          <form onSubmit={handleSubmit} className="space-y-3">
            {mode === "signup" && dniError && <p className="text-xs text-destructive">{dniError}</p>}
            <div>
