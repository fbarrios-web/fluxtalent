CREATE OR REPLACE FUNCTION public.get_public_vacancy_details(_slug text)
RETURNS TABLE(
  id uuid,
  title text,
  area text,
...
REVOKE ALL ON FUNCTION public.get_public_vacancy_details(text) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.get_public_vacancy_details(text) TO anon, authenticated, service_role;