DROP FUNCTION public.get_public_vacancy_details(text);
DROP FUNCTION public.get_public_vacancy_by_slug(text);

CREATE FUNCTION public.get_public_vacancy_by_slug(_slug text)
RETURNS TABLE(
  id uuid,
  title text,
  area text,
...
REVOKE ALL ON FUNCTION public.get_public_vacancy_by_slug(text) FROM PUBLIC;
